...
### [v0.0.2] アーキテクチャのリファクタリング(2026/06/22)
- **アーキテクチャレイヤー**：ServiceをUseCaseへ名称変更、DomainServiceを実装
- **ドメインモデル**：SleepRecordId型導入、就寝時間・起床時間のVO(SleepPeriod)によるカプセル化
- **エラーハンドリング**：Resultパターンの導入による例外駆動(CustomException)の廃止
- **DBマッピング**：PvtTrialsを別テーブルから同一テーブル内のカラム(JSON)へ変更
- **その他**：軽微なコード改修、単体テストの拡充(計36パターン)
